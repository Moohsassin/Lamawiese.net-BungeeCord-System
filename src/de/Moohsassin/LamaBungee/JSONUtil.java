...
public class JSONUtil {
	...
	public static String toJSON(String message) {
		...
		if (JSON_BUILDER.length() > RETAIN)	JSON_BUILDER.delete(RETAIN, JSON_BUILDER.length());
		...
		for (String part : parts) {
			...
			if (!ignoreFirst) newStyle = getStyle(part.charAt(0));
			else ignoreFirst = false;

			if (newStyle != null) {
				part = part.substring(1);
				if (newStyle.startsWith("\"c"))	colour = newStyle;
				else format = newStyle;
			}
			
			if (!part.isEmpty()) {
				if (first) first = false;
				else JSON_BUILDER.append(",");
				JSON_BUILDER.append("{");
	...
	private static String getStyle(char colour) {
		if (STYLE.length() > 0)	STYLE.delete(0, STYLE.length());
		switch (colour) {
		case 'k': return "\"obfuscated\": true,";
		case 'l': return "\"bold\": true,";
		case 'm': return "\"strikethrough\": true,";
		case 'n': return "\"underlined\": true,";
		case 'o': return "\"italic\": true,";
		case 'r': return "\"reset\": true,";
		default: break;
		}
